...
document._setMutationHandler(function(a) {
	...
	let tagName = '';
	if (a.node) {
		tagName = a.node.localName;
		for (var k in a.node._attrsByQName) {
			n[k] = a.node._attrsByQName[k].data;
		}
		n.innerHTML = a.node.innerHTML;
	}
	if (a.attr) {
		n[a.attr.localName] = a.attr.data;
	}
	opossum.mutated(a.type, ___path('yolo', a.target), tagName, n);
});
